...
class UniPCMultistepScheduler:
    ...
    def INPUT_TYPES(s): return {
        "required": {
            ...
            "predict_x0":               TRILEAN_WITH_DEFAULT,
            "solver_type":              (["default", "bh1", "bh2"], { "default": "default" }),
    ...
    def get(self, **kwargs):
        scheduler_config = { "scheduler": "UniPC" }
        for k, v in kwargs.items():
            if k in ["num_train_timesteps", "beta_start", "beta_end", "solver_order", "dynamic_thresholding_ratio", "sample_max_value", "steps_offset"]:
                scheduler_config[k] = v
            elif k in ["beta_schedule", "prediction_type", "solver_type", "timestep_spacing", "final_sigma_type"]:
